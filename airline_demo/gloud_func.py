...
class Uploader:
    ...
    def exist_check(self, date, blobpath):
        blobs = set(file_.name.replace('.zip', '') for file_ in 
                    self.bucket.list_blobs(prefix=blobpath))
        return date in blobs
...
def _local_cp(f_p):
    dst = os.path.join(os.getcwd(), os.path.basename(f_p))
    shutil.move(f_p, dst)
...
def main():
    ...
    uploader = Uploader('airline-demo')
    blobpath = 'tmp/zips/'
    ...
    if _size_check(f_p):
        # _local_cp(f_p) # for local test only
        blobname = f'tmp/zips/{os.path.basename(f_p)}'
        uploader.upload(f_p, blobname)
        os.remove(f_p)
    else:
